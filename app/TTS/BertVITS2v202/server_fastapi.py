...
from app.TTS.BertVITS2v202.tools.log import logger
from app.TTS.BertVITS2v202.infer import infer, get_net_g, latest_version
...
from app.TTS.BertVITS2v202.re_matching import cut_sent


from app.TTS.BertVITS2v202.config import config
...
for model_info in models_info:
    loaded_models.init_model(
        config_path=os.path.join(current_folder_path,model_info["config"]),
        model_path=os.path.join(current_folder_path,model_info["model"]),
...
@TTSRouter.get("/")
async def index():
    html_path = os.path.join(current_folder_path, "Web\\index.html")
    print(html_path)
...
def add_model(
    ...
    if config_path is None:
        model_path = os.path.join(current_folder_path,model_path.replace("/","\\"))
...
@TTSRouter.get("/models/get_unloaded")
...
@TTSRouter.get("/models/get_local")
def get_local_models_info(
    request: Request, root_dir: str = Query("Data", description="搜索根目录")
):
    root_dir = os.path.join(current_folder_path,"Data")
    """获取全部本地模型"""
    logger.info(
...
@TTSRouter.get("/tools/random_example")
def random_example(
    request: Request,
