...
async def record_microphone():
    ...
    print(type(args.chunk_size))
    print(type(args.chunk_interval))
    print(type(args.encoder_chunk_look_back))
    print(type(args.decoder_chunk_look_back))
    print(type(hotword_msg))
    print(type(use_itn))
    print(f"正在发送初始化信息, 信息:{message}")
    #voices.put(message)
...
async def ws_client(id, chunk_begin, chunk_size):
  ...
  for i in range(chunk_begin,chunk_begin+chunk_size):
    offline_msg_done=False
    voices = Queue()
    uri = "ws://{}:{}/{}".format(args.host, args.port,"recognition_wss/ali")
    print(uri)
